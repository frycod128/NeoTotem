...
模组只加入一个物品：永生图腾。
它存在于玩家原版物品栏的任意槽位时，将生命值下限稳定在 `1.0F` 并阻止标准死亡、基于原版事件的代码杀、`KILLED` 移除和死亡重生；
...
引导结束时叠加 6 秒失明 I，失败的触发会累加 1 秒失明时间。
...
产物位于 `build/libs/immortalitytotem-vX.X.X.jar`。客户端与服务端都应安装同一版本。
